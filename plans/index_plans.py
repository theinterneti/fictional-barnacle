...
def validate_spec_coverage(
    ...
    if specs_dir and specs_dir.is_dir():
        ...
        unreferenced = sorted(s for s in known_specs - all_refs if s <= "S17")
        if unreferenced:
            issues.append(
        ...
        if bad_refs:
            issues.append(f"Plans reference nonexistent specs: {', '.join(bad_refs)}")
    ...
    for p in plans:
        for ref in p.plan_refs:
            if ref not in known_files:
                issues.append(f"{p.file}: references {ref} which does not exist")
    return issues
...
def format_markdown(plans: list[PlanMeta]) -> str:
    ...
    lines.append("")
    lines.append(f"**{len(plans)} plans** indexed | Generated by `index_plans.py`")
    lines.append("")
    ...
    lines.append(f"- **Unique specs referenced**: {len(all_refs)}")
    lines.append(
        f"- **Plans with warnings**: {sum(1 for p in plans if p.warnings)}/{len(plans)}"
    )
    lines.append("")
...
def format_validation(plans: list[PlanMeta], specs_dir: Path | None = None) -> str:
