...
> **Level**: 3 — Platform
> **Dependencies**: S57 (Multi-Actor Universe Model), S58 (Turn Conflict Resolution), S10 (API and Streaming), S49 (Horizontal Scaling), S32 (NarrativeTransport)
...
```
WS /api/v1/games/{game_id}/universes/{universe_id}/ws
```

Authentication: The WS upgrade request MUST include the `session_token`
HttpOnly cookie set during S11 authentication. The server validates the JWT
from the cookie using the same logic as REST endpoints. The JWT MUST NOT be
passed as a query parameter — credentials in query strings appear in server
...
Following S49's header-based affinity pattern, the server MUST include the
`fly-force-instance-id` header in the WS upgrade response (Fly.io).
Reconnects and subsequent WS upgrade attempts MUST follow S49's defined
affinity and reconnect behavior; this spec does not require the client to echo
`fly-force-instance-id` on later requests. Cookies MUST NOT be used for
instance affinity.
...
A `WS /api/v1/games/{game_id}/universes/{universe_id}/ws` endpoint is added to the FastAPI application.
Connection authentication uses the same JWT validation as REST endpoints (via session cookie, not query param).

### FR-59.02 — Turn Submit via WebSocket

A `turn_submit` message triggers the same turn pipeline as a REST `POST /api/v1/games/{game_id}/turns`
...
Keepalive operates entirely at the application layer using the JSON `ping`/`pong`
messages defined in Section 4. The server responds with
`{type: "pong", payload: {}}` to every `{type: "ping", payload: {}}`.
Connections with no activity (no `ping`, no `turn_submit`) for > 60 seconds
receive a server-initiated `{type: "ping", payload: {}}` JSON message. If no
`{type: "pong", payload: {}}` is received within 10 seconds, the connection is
closed with WebSocket close code 1001. These are application-level messages;
WebSocket protocol-level ping/pong control frames (RFC 6455 §5.5.2–5.5.3) are
handled transparently by the underlying `websockets` library and are outside
this spec.
...
Feature: Multiplayer Transport
  ...
  Scenario: AC-59.01 — Actor connects and receives welcome event
    ...
    When Actor A opens a WebSocket to WS /api/v1/games/{game_id}/universes/{universe_id}/ws
