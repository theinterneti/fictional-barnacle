...
services:
  neo4j:
    ...
    ports:
      - 7687:7687
    options: >-
      --health-cmd "cypher-shell -u neo4j -p none 'RETURN 1' || exit 1"
      --health-interval 10s
      --health-timeout 5s
      --health-retries 10
```
...
A CI lint step SHALL validate fixture files with `cypher-lint` (or equivalent)
to catch Cypher syntax errors before test runs.
...
Feature: Live Neo4j in CI
  ...
  Scenario: AC-47.05 — world_full.cypher validates against S13 schema
    ...
    When cypher-lint (or equivalent) checks it
